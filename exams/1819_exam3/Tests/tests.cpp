...
TEST(test_1, test_e) {
	...
	if (crs1.size() > 0)
		//EXPECT_EQ("Tiago", crs1[0].getName()); //the original file had this line. However, the visiting is added to the c1 client, this is, Joao. When deleting clients, only the empty ones will be deleted, in this case, Tiago. So Joao will still be in the records.
